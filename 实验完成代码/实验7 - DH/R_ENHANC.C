...
int R_DigestInit (R_DIGEST_CTX *context, int digestAlgorithm)
{
...
int R_SignInit (R_SIGNATURE_CTX *context, int digestAlgorithm)
{
  return (R_DigestInit (&context->digestContext, digestAlgorithm));
}

int R_SignUpdate (R_SIGNATURE_CTX *context, unsigned char *partIn, unsigned int partInLen)
{
  return (R_DigestUpdate (&context->digestContext, partIn, partInLen));
}

int R_SignFinal (R_SIGNATURE_CTX *context, unsigned char *signature, unsigned int *signatureLen, R_RSA_PRIVATE_KEY *privateKey)
{
  int status;
...
int R_VerifyInit (R_SIGNATURE_CTX *context, int digestAlgorithm)
{
  return (R_DigestInit (&context->digestContext, digestAlgorithm));
}

int R_VerifyUpdate (R_SIGNATURE_CTX *context, unsigned char *partIn, unsigned int partInLen)
{
  return (R_DigestUpdate (&context->digestContext, partIn, partInLen));
}

int R_VerifyFinal (R_SIGNATURE_CTX *context, unsigned char *signature, unsigned int signatureLen, R_RSA_PUBLIC_KEY *publicKey)
{
  int status;
  unsigned char digest[MAX_DIGEST_LEN], digestInfo[DIGEST_INFO_LEN],
 ...
 */
int R_SealInit
  (R_ENVELOPE_CTX *context, unsigned char **encryptedKeys, unsigned int *encryptedKeyLens, unsigned char iv[8], 
   unsigned int publicKeyCount, R_RSA_PUBLIC_KEY **publicKeys,
 ...
 */
int R_SealFinal (R_ENVELOPE_CTX *context, unsigned char *partOut, unsigned int *partOutLen)
{
  unsigned int padLen;
 ...
 */
int R_OpenInit
  (R_ENVELOPE_CTX *context, int encryptionAlgorithm, unsigned char *encryptedKey, unsigned int encryptedKeyLen, 
  unsigned char iv[8], R_RSA_PRIVATE_KEY *privateKey)
{
  int status;
  ...
  do {
    if (context->bufferLen == 0)
      ...
      *partOutLen = 0;
      ...
      if (R_memcmp 
          ((POINTER)&lastPart[8 - padLen], PADDINGS[padLen], padLen) != 0) {
        status = RE_KEY;
        break;
      }
...
int R_SignPEMBlock 
  (unsigned char *encodedContent, unsigned int *encodedContentLen, unsigned char *encodedSignature, unsigned int *encodedSignatureLen,
   unsigned char *content, unsigned int contentLen, int recode, int digestAlgorithm, R_RSA_PRIVATE_KEY *privateKey)
{
  int status;
...
int R_SignBlock
  (unsigned char *signature, unsigned int *signatureLen, unsigned char *block, 
   unsigned int blockLen, int digestAlgorithm, R_RSA_PRIVATE_KEY *privateKey)
{
...
int R_VerifyPEMSignature 
  (unsigned char *content, unsigned int *contentLen, unsigned char *encodedContent, 
   unsigned int encodedContentLen, unsigned char *encodedSignature,
   unsigned int encodedSignatureLen, int recode, int digestAlgorithm, R_RSA_PUBLIC_KEY *publicKey)
{
  unsigned char signature[MAX_SIGNATURE_LEN];
  unsigned int signatureLen;
...
int R_VerifyBlockSignature 
  (unsigned char *block, unsigned int blockLen, unsigned char *signature, unsigned int signatureLen,
  int digestAlgorithm, R_RSA_PUBLIC_KEY *publicKey)
{
  R_SIGNATURE_CTX context;
  int status;
...
int R_SealPEMBlock 
  (unsigned char *encryptedContent, unsigned int *encryptedContentLen, unsigned char *encryptedKey, unsigned int *encryptedKeyLen,
   unsigned char *encryptedSignature, unsigned int *encryptedSignatureLen, unsigned char iv[8], unsigned char *content, unsigned int contentLen,
   int digestAlgorithm, R_RSA_PUBLIC_KEY *publicKey, R_RSA_PRIVATE_KEY *privateKey, R_RANDOM_STRUCT *randomStruct)
{
...
int R_OpenPEMBlock
  (unsigned char *content, unsigned int *contentLen, unsigned char *encryptedContent, 
   unsigned int encryptedContentLen, unsigned char *encryptedKey,
   unsigned int encryptedKeyLen, unsigned char *encryptedSignature, unsigned int encryptedSignatureLen,
   unsigned char iv[8], int digestAlgorithm, R_RSA_PRIVATE_KEY *privateKey, R_RSA_PUBLIC_KEY *publicKey)
{
  R_ENVELOPE_CTX context;
  int status;
  unsigned char encryptedKeyBlock[MAX_ENCRYPTED_KEY_LEN],
...
int R_DigestBlock (unsigned char *digest, unsigned int *digestLen, unsigned char *block, unsigned int blockLen, int digestAlgorithm)
{
 ...
 */
static void R_EncodeDigestInfo (unsigned char *digestInfo, int digestAlgorithm, unsigned char *digest)
{
  R_memcpy 
...
static void EncryptPEMUpdateFinal
  (R_ENVELOPE_CTX *context, unsigned char *output, unsigned int *outputLen, unsigned char *input, unsigned int inputLen)
{
...
static int DecryptPEMUpdateFinal (R_ENVELOPE_CTX *context, unsigned char *output, unsigned int *outputLen, 
...
static int CipherInit (R_ENVELOPE_CTX *context, int encryptionAlgorithm, unsigned char *key, unsigned char *iv, int encrypt)
{
 ...
 */
static void CipherUpdate (R_ENVELOPE_CTX *context, unsigned char *output, unsigned char *input, unsigned int len)
{
...
static void CipherRestart (R_ENVELOPE_CTX *context)
{
  if (context->encryptionAlgorithm == EA_DES_CBC)
